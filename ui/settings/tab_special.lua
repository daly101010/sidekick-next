...
function M.draw(settings, themeNames, onChange)
    ...
    specCell, changed = imgui.SliderInt('Cell Size', specCell, C.LAYOUT.MIN_CELL_SIZE, C.LAYOUT.MAX_CELL_SIZE)
    ...
    specRows, changed = imgui.SliderInt('Rows', specRows, 1, C.LAYOUT.MAX_ROWS)
    ...
    -- Anchor target
    local target = Settings.comboKeyed('Anchor To', settings.SideKickSpecialAnchorTarget or 'grouptarget', C.ANCHOR_TARGETS)
    ...
    specAnchor = Settings.comboString('Anchor Mode', specAnchor, C.ANCHOR_MODES)
    ...
    specAnchorGap, changed = imgui.SliderInt('Anchor Gap', specAnchorGap, 0, C.LAYOUT.MAX_ANCHOR_GAP)
