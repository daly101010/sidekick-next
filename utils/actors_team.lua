...
local function processPacket(entry, now)
    ...
    _peers[key] = {
        ...
        hpPct = tonumber(data.hpPct) or 0,
        manaPct = tonumber(data.manaPct) or 0,
        endurancePct = tonumber(data.endurancePct) or 0,
...
local function stateSignature(data)
    ...
    return table.concat({
        ...
        tostring(data.hpPct or 0),
        tostring(data.manaPct or 0),
        tostring(data.endurancePct or 0),
...
function M.tick(snapshot, settings)
    ...
    _local = {
        ...
        hpPct = tonumber(snapshot.hpPct) or 0,
        manaPct = tonumber(snapshot.manaPct) or 0,
        endurancePct = tonumber(snapshot.endurancePct) or 0,
