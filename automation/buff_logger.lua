...
function M.rotate()
    ...
    if _logFile then
        M.info('session', '=== Buff Session %s started ===', _sessionId)
    elseif err then
        print(string.format('[Buff Logger] Failed to open %s: %s', filename, err))
    end
end
...
function M.shutdown()
    if _logFile then
        M.info('session', '=== Buff Session %s ended ===', _sessionId)
        _logFile:close()
        _logFile = nil
    end
end
